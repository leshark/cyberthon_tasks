...
### Как решать: 
Давайте посмотрим на наш макрос. В нем определено разное поведение для вызова с одним аргументом, и с двумя и более
...
Посмотрим, что будет, если вызывать этот макрос с двумя, или более аргументами. 
```rs
($e:pat, $($es:pat),+) => {{
        let mut acc = calculate! { $($es),+ };
        acc.push(calculate!{$e}[0]);
